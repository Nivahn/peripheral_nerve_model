...
checkpointStep = 5.0; % save every ... ms
...
if (startFromCheckpoint)
    ...
    % --- НАЧАЛО: Экспорт stimTimes ---
    save('stimTimes_from_matlab.mat', 'stimTimes', 'spikePatterns'); % Также сохраним spikePatterns для согласованности
    disp('MATLAB_DEBUG: stimTimes и spikePatterns сохранены в stimTimes_from_matlab.mat');
    % --- КОНЕЦ: Экспорт stimTimes ---

    % --- НАЧАЛО: Отладочные принты MATLAB (инициализация) ---
    disp('--- MATLAB DEBUG: Initialisation ---');
    fprintf('N = %d, X = %f, dx = %f, numel(x) = %d\n', N, X, dx, numel(x));
    disp(['x(1:min(5,end)): ', mat2str(x(1:min(5,end)))]);

    num_axons_to_print = min(3, N);
    disp('stimTimes (первые несколько стимулов для первых нескольких аксонов):');
    for i = 1:num_axons_to_print
        fprintf('  Axon %d (%s): ', i, spikePatterns{i});
        fprintf('%f ', stimTimes{i}(1:min(5, numel(stimTimes{i}))));
        fprintf('\n');
    end

    disp(['Size of V: ', mat2str(size(V))]);
    disp(['Size of I: ', mat2str(size(I))]);
    disp(['Size of tau: ', mat2str(size(tau))]);
    disp(['Size of lamb: ', mat2str(size(lamb))]);
    disp(['Size of mask: ', mat2str(size(mask))]);

    if N > 0 && numel(x) > 0
        num_points_to_print = min(5, numel(x));
        disp('Initial V (first axon, few points):');
        disp(V(1, 1:num_points_to_print));
        disp('Initial mask (first axon, few points):');
        disp(mask(1, 1:num_points_to_print));
        disp('Initial tau (first axon, few points):');
        disp(tau(1, 1:num_points_to_print));
        disp('Initial lamb (first axon, few points):');
        disp(lamb(1, 1:num_points_to_print));
    end
    disp('--- MATLAB DEBUG: End Initialisation ---');
