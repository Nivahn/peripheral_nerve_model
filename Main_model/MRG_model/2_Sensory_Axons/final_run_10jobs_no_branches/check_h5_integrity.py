...
EXPECTED_SCENARIOS = ["no_branch_reference"]
...
def prefix_for(topology: str, scenario: str) -> str:
    if topology == "one_node_branching":
        if scenario == "no_branch_reference":
            return "on_nb"
        raise ValueError(f"Unsupported no-branch scenario: {scenario}")
    raise ValueError(f"Unsupported no-branch topology: {topology}")
