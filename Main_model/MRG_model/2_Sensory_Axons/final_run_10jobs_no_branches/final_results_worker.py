...
DEFAULT_SCENARIO_NAME = "no_branch_reference"
...
AXON_B_NO_BRANCH_REFERENCE_LABELS = ["stimulation_point", "before_branch", "branch_point", "after_branch_main", "terminal_main"]
AXON_BRANCH_LABELS = [
    "stimulation_point",
    "before_branch",
    "after_branch_main",
...
def build_launch_config(topology_name: str, fiber_diameter_um: float, scenario_name: str, *, test_mode: bool = False) -> LaunchConfig:
    ...
    if scenario_name not in {"no_branch_reference"}:
    ...
    if scenario_name == "no_branch_reference":
        # No branches: same 27-node main path as the branched baseline, with pseudo branch recording points.
        return LaunchConfig(
            topology_name=topology_name,
            branch_topology_mode=branch_topology_mode,
            fiber_diameter_um=float(fiber_diameter_um),
            scenario_name=scenario_name,
            scenario_dir="no_branch_reference",
            prefix="cb_nb" if topology_name == "connector_branching" else "on_nb",
    ...
    raise ValueError(f"Unsupported no-branch scenario_name={scenario_name}")
...
def write_root_metadata(
    ...
    *,
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        f.attrs["created_by"] = "final_run_10jobs_no_branches/final_results_worker.py"
...
def build_model(cfg: LaunchConfig, edge_dist_um: float, mode_name: str, *, h_stop_ms: float | None = None) -> TwoSensoryAxonsPrescott:
    return TwoSensoryAxonsPrescott(
        ...
        pseudo_branch_recording_A=cfg.scenario_name == "no_branch_reference",
        pseudo_branch_recording_B=cfg.scenario_name == "no_branch_reference",
        pseudo_branch_node_index=8,
        pseudo_before_offset_nodes=3,
        pseudo_after_offset_nodes=3,
    )
...
def run_one_file(
    ...
    *,
    ...
    for freq_hz in cfg.frequencies_hz:
        ...
        axon_b_labels = AXON_B_NO_BRANCH_REFERENCE_LABELS
